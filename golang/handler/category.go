...
func (h *CategoryHandler) GetEntity(result []*model.CategoryResult) map[string]*resp.Category {
	...
	for _, item := range result {
		...
		for _, val := range c3map {
			if item.C2CategoryID == val.ParentID {
				secondCategory.Children = val
			}
		}
	...
	for _, item := range result {
		...
		for _, val := range c2map {
			if item.C1CategoryID == val.ParentID {
				firstCategory.Children = val
			}
		}
